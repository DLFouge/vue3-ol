...
import {addImage,addImageR,imageLayerR} from '@/components/AddImage'
...
export default defineComponent({
    setup() {
        ...
        onMounted(() => {
            ...
            addImage(map, '/src/assets/4.png', coord);
        });
        ...
        const moveTop = () => {
            ...
            let obj = <any>{};
            LayerSwipesTop(rightLayer);
            if(imageLayerR){
                LayerSwipesTop(imageLayerR);
            }
            //重新渲染图层
            map.render();
            swipe.onmousedown = (event:any) => {
        ...
        const onDrop = async(e:any) => {
            ...
            if(isLeft.value){
                if(e.offsetX <= swipe.offsetLeft){
                    //左侧
                    addImage(map, dragItem.src, dragItem.coord);
